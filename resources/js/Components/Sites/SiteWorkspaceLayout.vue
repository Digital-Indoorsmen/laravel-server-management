...
const tabs = [
    {
        key: "overview",
        label: "Overview",
        routeName: "sites.workspace.overview",
    },
    {
        key: "deployments",
        label: "Deployments",
        routeName: "sites.workspace.deployments",
    },
    {
        key: "processes",
        label: "Processes",
        routeName: "sites.workspace.processes",
    },
    {
        key: "commands",
        label: "Commands",
        routeName: "sites.workspace.commands",
    },
    ...
    { key: "domains", label: "Domains", routeName: "sites.workspace.domains" },
    {
        key: "settings",
        label: "Settings",
        routeName: "sites.workspace.settings",
    },
];
</script>
...
<template>
    ...
    <AppLayout>
        <div class="space-y-6">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="space-y-2">
                    <div class="flex items-center gap-3">
                        <Link
                            ...
                            aria-label="Back to server sites"
                        >
                            <svg
                ...
                <div class="flex flex-wrap items-center gap-2 text-xs">
                    <span class="badge badge-outline"
                        >Server: {{ site.server.name }}</span
                    >
                    <span class="badge badge-outline"
                        >IP: {{ site.server.ip_address }}</span
                    >
                    <span class="badge badge-outline"
                        >Site ID: {{ site.id }}</span
                    >
                </div>
            </div>
            ...
            <div
                role="tablist"
                aria-label="Site workspace tabs"
                class="tabs tabs-boxed w-full gap-1 overflow-x-auto bg-base-100 p-2"
            >
                <Link
                    v-for="tab in tabs"
                    :key="tab.key"
